                                ...
                                @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                                    <div class="tab-pane {{ $localeCode == config('kakhura.site-bases.admin_editors_default_locale') ? 'active' : '' }}" id="{{ $localeCode }}">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-md-10 col-sm-10 col-xs-12">
                                                <textarea id="description_min_{{ $localeCode }}" class="textarea" name="description_min_{{ $localeCode }}">{{ $news->detail()->where('locale', $localeCode)->first()->description_min }}</textarea>
                                            </div>
                                        </div>
                                    </div>
                                @endforeach
